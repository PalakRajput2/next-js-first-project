...
const CategoryPage = () => {
  ...
  return (
    <section className="px-20 py-12">
      {/* Tabs */}
      <div className="flex flex-wrap justify-center gap-10  pb-0 mb-10">
        {categories.map((cat) => (
          <button
            key={cat.id}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 pt-10">
        {filtered.map((product) => (
          <div key={product.id} className="relative group">
            ...
            {product.badge && (
              <span
                className={`absolute top-3 left-3 px-3 py-2 text-xs font-semibold uppercase rounded ${
                  product.badge === "Sale"
